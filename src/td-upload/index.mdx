...
|:--|:--|:--|:--|
|files|原数据|Array|[]|
|filterOptions|自定义数据过滤，需要返回过滤后的数据对象。返回的对象中必须包含 `name、uid、filePath`|function(item, index)|-|
...
|参数|说明|类型|默认值|
|:--|:--|:--|:--|
|fileList|渲染的文件列表原数据，需要使用 onInitialFiles 进行过滤；或者数据对象中必须包含 `name、url`|array|-|
