...
```
noPassByMobile('13812345678'); // 138****5678
```
...
dva.routerRedux.push 的简单封装，实现路由切换

```
// 全局配置，需要app全局初始化完成后进行设置，调用 store.dispatch 进行重定向
redirect.config({
  appStore: app, // 包裹应用的 app._store 对象
})

// 路由切换，如切换至：localhost:8080/demo
redirect.to('/demo', dispatch); // 第二个参数 dispatch 如果不传，将会从全局配置项中获取
```